...
from .camera_interaction_toolkit.camera_model import CameraModel

#from camera_interaction_toolkit.camera.camera_model import CameraModel
import json
import os
...
class CalibraterUtility:
	...
	def undistort_points(cam_mat, dist, pts, imsize):
		...
		temp = cv2.undistortPoints(pts.copy(), cam_mat, dist, None, new_cam_mat)
	...
	def setup_cameras(cam_dict):
		...
		for k in cam_dict.keys():
			curr_cam =  cam_dict[k]
			CameraIntrinsic = (np.array(curr_cam['CameraIntrinsic'])).reshape((3,3))
			CameraMatrix = (np.array(curr_cam['CameraMatrix'])).reshape((4,4))
			DistCoeff = np.array(curr_cam['DistCoeff'])
			cam_model_dict[k] = CameraModel(CameraIntrinsic, CameraMatrix, DistCoeff)
